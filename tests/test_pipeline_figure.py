...
def test_contracts_reference_real_sidecars(gen, registry) -> None:
    ...
    for producer, sidecar, consumer in flows:
        ...
        # The consumer of every contract is a real downstream node: the
        # deterministic renderer or one of the verify gates.
        assert consumer in set(gen._GATE_NODES) | {gen._RENDERER_NODE}
...
def test_honest_depiction_real_gates_and_renderer(gen) -> None:
    """Honesty constraint: depict E2ER's REAL assurance — the two gates that
    actually run before reviewers (verify_numbers + verify_citations, both in
    runner._run_review_phase) and the deterministic table renderer — and do NOT
    invent capabilities E2ER lacks (cross-model review)."""
    dot = gen.build_dot().lower()
    assert "verify_numbers" in dot
    assert "verify_citations" in dot  # real M2 gate — src/core/pipeline/verify_citations.py
    assert "table_renderer" in dot  # deterministic results-table renderer
    # E2ER uses SAME-model reviewers — must not claim cross-model.
    assert "cross-model" not in dot
    assert "cross_model" not in dot


def test_table_spec_consumed_by_renderer(gen, registry) -> None:
    """The deterministic-tables story: if table_spec.json is a declared sidecar,
    its consumer in the figure is the renderer (not a verify gate)."""
    declared: set[str] = set()
    for files in registry.SPECIALIST_SIDECAR_ARTIFACTS.values():
        declared.update(files)
    if "table_spec.json" not in declared:
        pytest.skip("table_spec.json is not a declared sidecar in this revision")
    consumers = {c for _p, s, c in gen._contract_flows() if s == "table_spec.json"}
    assert consumers == {gen._RENDERER_NODE}
